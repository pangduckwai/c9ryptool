...
func main() {
	...
	case "crenc":
		...
		buff, err := utils.Read("test/cr.pem", 32768)
		if err != nil {
			log.Fatalf("[TEST][%v] %v", cmd, err)
		}
	...
	case "encdec":
		...
		buff, err := utils.Read("test/self.key", 32768)
		if err != nil {
			log.Fatalf("[TEST][%v] %v", cmd, err)
		}
	...
	case "eckeyread":
		...
		prvb, err := utils.Read("test/c9.pem", 65535)
		if err != nil {
			log.Fatalf("[TEST][%v][0] %v", cmd, err)
		}
		key, err := parse(prvb)
		...
		pubb, err := utils.Read("test/c9-pub.pem", 65535)
		if err != nil {
			log.Fatalf("[TEST][%v][1] %v", cmd, err)
		}
